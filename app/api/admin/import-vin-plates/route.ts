...
// One-time import endpoint — DELETE THIS FILE after running
// Usage: POST /api/admin/import-vin-plates  (no auth needed, run once then delete)
...
export async function POST() {
  try {
    ...
    const existing = await prisma.vinPlateRecord.count()
    if (existing > 0) {
      return NextResponse.json({ error: `Already has ${existing} records — import aborted to avoid duplicates` }, { status: 409 })
    }
